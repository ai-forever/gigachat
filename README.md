...
Дополнительно вы можете задать собственные HTTP-заголовки с помощью параметра `headers`. Значения из `headers` будут добавлены ко всем запросам, а при вызове методов `chat` или `stream` можно передать свои заголовки, которые переопределят заголовки, заданные при инициализации.
