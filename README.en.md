...
- [Usage Examples](#usage-examples)
  ...
  - [More Examples](#more-examples)
    - [Simple chat loop (multi-turn)](#simple-chat-loop-multi-turn)
    - [Retrieve an image by file ID](#retrieve-an-image-by-file-id)
    - [AI check (detect AI-generated text)](#ai-check-detect-ai-generated-text)
...
#### Simple chat loop (multi-turn)

An interactive multi-turn chat loop that keeps conversation state:
...
chat = Chat(
    messages=[
        Messages(
            ...
            content="You are a careful assistant that helps the user solve their problems.",
        )
    ],
    temperature=0.7,
    max_tokens=100,
...
with GigaChat() as client:
    while True:
        ...
        chat.messages.append(Messages(role=MessagesRole.USER, content=user_input))
        response = client.chat(chat)
        chat.messages.append(response.choices[0].message)
...
Check whether a text appears to be AI-generated and get the detected AI-written intervals:

```python
from gigachat import GigaChat

text = "Cats are wonderful companions. They are playful, curious, and love attention."

with GigaChat() as client:
    result = client.check_ai(text, model="GigaCheckDetection")
