...
* [Primary Chat](./example_chat.py) - Sync `client.chat.create(...)` and `client.chat.stream(...)` examples for the primary `chat/completions` surface
...
* [Structured Output](./example_structured_output.ipynb) - Getting structured JSON responses using JSON Schema, including the legacy parse helper during migration
