...
1. Используйте `client.chat(...)` для старого контракта или `client.chat.create(...)` для v2.
2. Используйте `client.stream(...)` для старого контракта или `client.chat.stream(...)` для v2.
3. Используйте `client.chat_parse(...)` для старого контракта или `client.chat.parse(...)` для v2.
...
`client.chat(...)`, `client.stream(...)`, `client.chat_parse(...)`, `client.achat(...)`, `client.astream(...)` и `client.achat_parse(...)` продолжают работать как compatibility shim-ы для старого chat-контракта. Они не deprecated и не вызывают `DeprecationWarning`.
...
- root compatibility methods сохраняют старый контракт доступным, а явные resource methods открывают v2
...
2. Если payload пока сложно переносить, продолжайте использовать root compatibility methods.
   Это сохраняет старый контракт доступным без немедленного полного переписывания.
...
- Если вы хотите остаться на старом контракте, используйте `client.chat_parse()` и `client.achat_parse()`.
- Передача Pydantic-модели в `response_format` у старого create-пути не является заменой миграции; в этом сценарии нужно использовать `parse()`.
...
## Если вы пока остаетесь на старом контракте
...
- нужно сначала явно обозначить использование старого контракта
...
Относитесь к root compatibility methods как к режиму старого контракта, а не как к финальной точке для нового v2-кода.
...
- Передавать Pydantic-модель в старый create-путь вместо перехода на `parse()`.
