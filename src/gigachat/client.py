...
from gigachat.api import post_chat, post_oauth, post_token, stream_chat
...
class _BaseClient:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        config = {key: value for key, value in locals().items() if key not in ("self", "kwargs") and value is not None}
        self._settings = Settings(**config)
        if self._settings.token:
            self._access_token = AccessToken(access_token=self._settings.token, expires_at=0)
