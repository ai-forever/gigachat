...
class GigaChatSyncClient(_BaseClient):
    ...
    def validate_function(
        self,
        function: Union[CustomFunction, Function, ChatFunctionSpecification, Dict[str, Any]],
    ) -> FunctionValidationResult:
        """Validate a GigaChat function description."""
        return tools.functions_validate_sync(self._client, function=function, access_token=self.token)

    @_with_retry
    @_with_auth
    def check_ai(self, text: str, model: AICheckModel) -> AICheckResult:
...
class GigaChatAsyncClient(_BaseClient):
    ...
    @_awith_retry
    @_awith_auth
    async def avalidate_function(
        self,
        function: Union[CustomFunction, Function, ChatFunctionSpecification, Dict[str, Any]],
    ) -> FunctionValidationResult:
        """Validate a GigaChat function description."""
        return await tools.functions_validate_async(self._aclient, function=function, access_token=self.token)

    @_awith_retry
    @_awith_auth
    async def acheck_ai(self, text: str, model: AICheckModel) -> AICheckResult:
        """Check the provided text for content generated by AI models."""
