...
class GigaChatSyncClient(_BaseClient):
    ...
    @_with_retry
    @_with_auth
    def filter_check(self, payload: FilterCheckRequest) -> FilterCheckResult:
        """Check messages against thematic restrictions."""
        return tools.filter_check_sync(self._client, payload=payload, access_token=self.token)
...
class GigaChatAsyncClient(_BaseClient):
    ...
    @_awith_retry
    @_awith_auth
    async def afilter_check(self, payload: FilterCheckRequest) -> FilterCheckResult:
        """Check messages against thematic restrictions."""

        return await tools.filter_check_async(self._aclient, payload=payload, access_token=self.token)
