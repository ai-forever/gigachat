...
from gigachat.models.chat import Function, Messages
...
class FilterCheckSettings(BaseModel):
    """Content filter settings."""

    neuro: bool = Field(default=True, description="Enable checks with the neural filtering model.")
    blacklist: bool = Field(default=True, description="Enable checks against the stop-word blacklist.")
    whitelist: bool = Field(default=True, description="Enable checks against the prepared-response whitelist.")


class FilterCheckRequest(BaseModel):
    """Content filter request."""

    model: str = Field(default="GigaFilter", description="Name of the filtering model.")
    messages: List[Messages] = Field(description="Messages to check.")
    settings: Optional[FilterCheckSettings] = Field(default=None, description="Optional filtering settings.")
    functions: Optional[List[Function]] = Field(default=None, description="Functions available to the conversation.")


class FilterCheckUsage(BaseModel):
    """Content filter usage statistics."""

    filter_tokens: int = Field(description="Number of tokens processed by the filter.")


class FilterCheckResult(APIResponse):
    """Content filter result."""

    is_profane: bool = Field(description="Whether the messages match thematic restrictions.")
    usage: FilterCheckUsage = Field(description="Content filter usage statistics.")
