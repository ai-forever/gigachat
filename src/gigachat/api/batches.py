...
def get_batch_content(file: FileContent) -> bytes:
    """Read batch JSONL input into a retry-safe bytes snapshot."""
...
def _create_batch_kwargs(
    *,
    ...
    return {
        "method": "POST",
        "url": "/batches",
        "content": get_batch_content(file),
...
def _normalize_batches_payload(payload: Union[Dict[str, Any], List[Dict[str, Any]]]) -> Dict[str, Any]:
...
def _build_batches_response(response: httpx.Response) -> Batches:
    ...
    payload = _normalize_batches_payload(response.json())
