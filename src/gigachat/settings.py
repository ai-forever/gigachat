...
class Settings(BaseSettings):
    ...
    def compute_oauth_token(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if values["use_auth"]:
            use_secrets = values["oauth_token"] or values["token"] or (values["user"] and values["password"])
            if not use_secrets:
                logging.warning("Please provide GIGA_CLIENT_ID and GIGA_CLIENT_SECRET" " environment variables.")
